...
const TRACK_QUESTIONS = [
  ...
  },
  {
    ...
    options: ['Complete beginner', 'Know some basics', 'Can hold simple conversations', 'Quite comfortable', 'Advanced / near fluent'],
    levels: ['a1', 'a2', 'b1', 'b2', 'c1'],
    weights: { travel: [1,1,1,1,1], business: [1,1,1,1,1], study: [1,1,1,1,1] },
  },
];
...
function PlacementInner() {
  ...
  const computeCefrResult = (ans: (number|null)[]) => {
    ...
    // 2) 퀴즈 기반 레벨 계산 (각 레벨 50% 이상 맞추면 통과)
    let quizResult = 'a1';
    for (const lvl of ['a1','a2','b1','b2','c1']) {
      const total = questions.filter(q => q.level === lvl).length || 2;
      if (scores[lvl] >= Math.ceil(total * 0.5)) quizResult = lvl;
      else break;
    }

    // 3) C2 달성 조건: C1 문제 전부 정답
    const c1Total = questions.filter(q => q.level === 'c1').length;
    if (c1Total > 0 && scores['c1'] === c1Total) quizResult = 'c2';
    ...
    // 5) 신고레벨은 40~79% 구간에서만 tie-breaker (+1 보정)
    // 0~19%  : 퀴즈 결과 그대로 (실력 없음 그대로 반영)
    // 20~39% : 퀴즈 결과 그대로
    // 40~59% : 신고레벨이 퀴즈결과보다 2단계 이상 높으면 +1
    // 60~79% : 신고레벨이 퀴즈결과보다 1단계 이상 높으면 +1
    // 80~100%: 퀴즈 결과 그대로 (높은 실력 그대로 반영)
    let result = quizResult;
    const hintIdx = order.indexOf(trackResult?.levelHint || 'a1');
    const quizIdx = order.indexOf(quizResult);

    if (accuracy >= 0.4 && accuracy < 0.6) {
      // 40~59%: hint가 퀴즈보다 2단계 이상 높을 때만 +1
      if (hintIdx >= quizIdx + 2) {
        result = order[Math.min(quizIdx + 1, order.length - 1)];
      }
    } else if (accuracy >= 0.6 && accuracy < 0.8) {
      // 60~79%: hint가 퀴즈보다 1단계 이상 높을 때만 +1
      if (hintIdx >= quizIdx + 1) {
        result = order[Math.min(quizIdx + 1, order.length - 1)];
      }
    }
